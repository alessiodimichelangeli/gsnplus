README for TelosbSensorReading 
...
TelosbSensorReading is a simple sensors-collection app.
...
by GSN+ Server.  
This app also allows to receive a message from a gsn server.
A message could:
- modify the sample interval
- send commands (in the implemented example these are: turn on red led, turn off red led, blink red led).   


**** HOW TO COMPILE: **** 

You can compile TelosbSensorReading with a telosb mote by compiling
as follows (from the TelosbSensorReading folder):	
...
To display the readings from TelosbSensorReading motes, install the 
TelosbBaseStation app on a mote connected to your PC's serial port.
(see the related README.txt of that app)

Then, run the following command to generate the message:
--> mig -target=null -java-classname=TelosbSensorReadingMsg java TelosbSensorReading.h telosbsensorreading -o > TelosbSensorReadingMsg.java
...
At:
--> localhost:22001
you should see the gsn server so load the TelosbSensorReading Virtual Sensor
