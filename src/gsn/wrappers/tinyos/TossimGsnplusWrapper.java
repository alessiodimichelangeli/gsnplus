/*
* File: src/gsn/wrappers/tinyos/TossimGsnplusWrapper.java
*
...
//THIS WRAPPER IS USEFULL TO SIMULATE THE TINYOS INTEGRATION VIA TOSSIM.
//It reads datas from the TossimGsnplusMsg ans it can manage the message upload from the GSN SERVER to TinyOs.
//In particular, it can upload this command:
//	- reset counter at
...
This wrapper is not very different from the TinyOs Wrapper. 
This simply read and write a message with one field (counter). 
The power is from the TinyOs side, in particular the Tossim Live simulation, 
that it allows to test these functions without having the real sensors.
...
public class TossimGsnplusWrapper extends AbstractWrapper implements net.tinyos.message.MessageListener {
	...
	*/
	...
	//YOU HAVE TO EDIT THIS
  	private String packetName="TossimGsnplusMsg"; //this must be the name of the msg file generated by mig 
  	//
  	
  	private String path="/home/alessio/gsnplus/src/gsn/wrappers/tinyos/"; //this must be the path of the file generated by mig
  	...
  	public boolean sendToWrapper(String action, String[] paramNames, Object[] paramValues) throws OperationNotSupportedException {
		   try 	{
				...
				*/
				...
				TossimGsnplusMsg tgsn= new TossimGsnplusMsg(); 
				
				
				if (action.equals("reset_counter_at")){	
					...
					tgsn.set_counter(updatedFirstField);	
				}
				
				
    				
    				//Finally send the message with the updated data via broadcast to the virtual motes (TOSSIM)
   				moteIF.send(0, tgsn);
   				//
